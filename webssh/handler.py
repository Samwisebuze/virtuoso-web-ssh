...
import webssh.get_virtuoso_container as get_virtuoso_container
...
class IndexHandler(MixinHandler, tornado.web.RequestHandler):
    ...
    def get_hostname(self):
        ...
        return self.get_value('hostname')
    ...
    def get_args(self):
        # Get all config information from http request.
        hostname = self.get_hostname()

        # hostname is actually the docker container id
        docker_container_id = hostname

        # get the real hostname from the docker container id
        hostname = get_virtuoso_container.get_virtuoso_hostname(docker_container_id)

        # get the corresponding port for the container id
        port = get_virtuoso_container.get_virtuoso_port(docker_container_id)

        logging.info(f'Connecting to Docker container at {hostname}:{port}')
        
        # hardcode the username and password (this is bad, but it works for now)
        username = os.getenv('STANDARD_CONTAINER_USERNAME', '')
        password = os.getenv('STANDARD_CONTAINER_PASSWORD', '')
